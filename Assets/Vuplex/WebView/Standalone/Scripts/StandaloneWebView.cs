...
namespace Vuplex.WebView {
    ...
    public abstract partial class StandaloneWebView : BaseWebView,
        ...
        /// <summary>
        /// Deletes all cookies for all URLs. This method can only be called after one or more webviews have been initialized.
        /// </summary>
        /// <example>
        /// <code>
        /// #if UNITY_STANDALONE
        ///     StandaloneWebView.DeleteAllCookies();
        /// #endif
        /// </code>
        /// </example>
        ...
        public static Task<Cookie[]> GetCookies(string url, string cookieName = null) {
            ...
            _assertBrowserProcessIsRunning("GetCookies");
        ...
        public async Task Init(int width, int height) {
            ...
            _nativeWebViewPtr = WebView_new(gameObject.name, width, height, null);
        ...
        /// </summary>
        /// <remarks>
        /// This method can only be called prior to initializing any webviews. So, you will likely need to call it from Awake().
        /// </remarks>
        /// <example>
        /// <code>
        /// void Awake() {
        ...
        public static Task<bool> SetCookie(Cookie cookie) {
            ...
            _assertBrowserProcessIsRunning("SetCookie");
            if (cookie == null) {
        ...
        public static void TerminatePlugin() => WebView_terminatePlugin();

    #region Non-public members
        delegate void GetCookiesCallback(string requestId, string serializedCookies);
        delegate void ModifyCookiesCallback(string requestId, bool success);
        ...
        static void _assertBrowserProcessIsRunning(string methodName) {

            if (!WebView_browserProcessIsRunning()) {
                throw new InvalidOperationException($"On Windows and macOS, {methodName}() can only be called when the Chromium process is running (i.e. after a webview has been initialized).");
            }
        }

        static Task<bool> _deleteCookies(string url = null, string cookieName = null) {

            _assertBrowserProcessIsRunning("DeleteCookies");
            var taskSource = new TaskCompletionSource<bool>();
            var resultCallbackId = Guid.NewGuid().ToString();
            _pendingModifyCookiesResultCallbacks[resultCallbackId] = taskSource.SetResult;
        ...
        [AOT.MonoPInvokeCallback(typeof(GetCookiesCallback))]
        ...
        void HandlePopup(string message) {
            ...
            Dispatcher.RunOnMainThread(async () => {
                await popupWebView._initPopup(Size.x, Size.y, popupBrowserId);
                PopupRequested?.Invoke(this, new PopupRequestedEventArgs(url, popupWebView as IWebView));
            });
        }

        [AOT.MonoPInvokeCallback(typeof(ModifyCookiesCallback))]
        ...
        async Task _initPopup(int width, int height, string popupId) {

            await _initBase(width, height, asyncInit: true);
            _nativeWebViewPtr = WebView_new(gameObject.name, width, height, popupId);
            await _initTaskSource.Task;
        }
        ...
        static void _initializePlugin() {
            ...
            WebView_initializePlugin(
                Marshal.GetFunctionPointerForDelegate<UnitySendMessageFunction>(_unitySendMessage),
                Marshal.GetFunctionPointerForDelegate<GetCookiesCallback>(_handleGetCookiesResult),
                Marshal.GetFunctionPointerForDelegate<ModifyCookiesCallback>(_handleModifyCookiesResult)
            );
        ...
        static void _throwAlreadyInitializedException(string methodName) {
            ...
            throw new InvalidOperationException($"Unable to execute {methodName}() because a webview has already been created. On Windows and macOS, {methodName}() can only be called prior to initializing any webviews. So, please call {methodName}() earlier in the application, for example, by calling it from Awake() instead of from Start().");
        }
        ...
        [DllImport(_dllName)]
        static extern void WebView_initializePlugin(IntPtr unitySendMessageFunction, IntPtr getCookiesCallback, IntPtr modifyCookiesCallback);
